...
int controller_sortPassenger(LinkedList* pArrayListPassenger)
{
	...
	Passenger* comprobar;
	Passenger* segundoComprobar;

	int indexUno;
	int indexDos;

	for(comprobar = indice->pElement; indice != NULL; indice = indice->pNextNode)
	{

		comprobar = indice->pElement;
		if(indice->pNextNode != NULL)
		{
			segundoIndice = indice->pNextNode;
			for(segundoComprobar = segundoIndice->pElement; segundoIndice != NULL; segundoIndice = segundoIndice -> pNextNode)
			{
				segundoComprobar = segundoIndice->pElement;
				if(comprobar->id > segundoComprobar->id)
				{
					indexUno = ll_indexOf(pArrayListPassenger, comprobar);
					indexDos = ll_indexOf(pArrayListPassenger, segundoComprobar);
					ll_set(pArrayListPassenger, indexUno, segundoComprobar);
					ll_set(pArrayListPassenger, indexDos, comprobar);
					retorno = 1;
				}
